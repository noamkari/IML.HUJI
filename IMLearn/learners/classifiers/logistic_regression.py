...
class LogisticRegression(BaseEstimator):
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        if self.penalty_ == "l1":
            self.model = RegularizedModule(LogisticModule(np.random.normal(w)),
                                           L1(w),
    ...
    def predict_proba(self, X: np.ndarray) -> np.ndarray:
        ...
        # prb = 1 / (1 + np.exp(-self.predict(X)))
        # return np.where(prb > self.alpha_, 1, 0)
        #
        return 1 / (1 + np.exp(- X @ self.model.weights_))
